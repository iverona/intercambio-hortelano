...
const ExchangeItem = ({ exchange, onClick }: { exchange: Exchange; onClick: () => void }) => {
 ...
 const getStatusColor = (status: string) => {
  switch (status) {
   case 'pending':
    return 'bg-card text-foreground dark:text-card-foreground border-[#D4CFAE] dark:border-[#5e5a4b]';
   case 'accepted':
    return 'bg-muted/40 text-foreground /40 dark:text-card-foreground border-[#A6C6B9]/50';
   case 'rejected':
    return 'bg-secondary/20 text-[#5e4043] dark:bg-[#7a6466]/40 border-secondary/30';
   case 'completed':
    return 'bg-primary/20 text-foreground dark:bg-muted/40 border-primary/30';
   default:
 ...
 return (
  <Card
   onClick={onClick}
   className="p-4 md:p-5 hover:shadow-xl transition-all duration-300 cursor-pointer hover:scale-[1.02] bg-card border border-card group relative overflow-hidden"
  >
   {/* Hover glow effect */}
   <div className="absolute inset-0 bg-muted/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>

   <div className="flex items-start gap-3 md:gap-4 relative z-10">
    {/* Avatar */}
    <Avatar className="h-12 w-12 md:h-14 md:w-14 border-2 border-card shadow-md flex-shrink-0">
     <AvatarImage src={exchange.partner?.avatarUrl} alt={exchange.partner?.name} />
     <AvatarFallback className="bg-secondary dark:bg-[#6b585a] text-white text-lg font-bold">
    ...
    <div className="flex-1 min-w-0">
     <div className="flex items-start justify-between mb-1 md:mb-2 gap-2">
      <div className="min-w-0 flex-1">
       <h3 className="font-bold text-base md:text-lg text-foreground truncate font-display">
        {exchange.productName}
       </h3>
       <p className="text-xs md:text-sm text-primary truncate font-serif">
     ...
     {/* Offer details */}
     {exchange.offer && (
      <div className="flex items-center gap-2 mb-2 md:mb-3 p-2 bg-background rounded-lg border border-card ">
       <div className="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm text-foreground min-w-0">
        <span className="flex-shrink-0 text-primary dark:text-[#556B2F]">{getOfferTypeIcon(exchange.offer.type)}</span>
     ...
     <div className="flex items-center justify-between gap-2">
      {exchange.lastMessage ? (
       <p className="text-xs md:text-sm text-primary truncate flex-1 flex items-center gap-1 font-serif">
        <MessageSquare className="w-3 h-3 flex-shrink-0" />
        <span className="truncate">{exchange.lastMessage.text}</span>
       </p>
      ) : (
       <p className="text-xs md:text-sm text-primary italic truncate flex-1 font-serif">
        {exchange.status === 'pending' && !isRequester
      ...
      <div className="flex items-center gap-1 md:gap-2 text-xs text-secondary dark:text-[#7a6466] flex-shrink-0">
...
const ExchangeSkeleton = () => (
 <div className="space-y-3 md:space-y-4">
  {[...Array(3)].map((_, i) => (
   <Card key={i} className="p-4 md:p-5 bg-card border-gray-100 dark:border-gray-700">
  ...
  <OrganicBackground className="pb-20 md:pb-0">
   <div className="container mx-auto px-4">
    ...
    <div className="max-w-4xl mx-auto py-8">
     <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-8">
      <div className="flex items-center gap-3 md:gap-4">
       ...
       <div className="min-w-0">
        ...
        <p className="text-sm md:text-base text-primary mt-1 truncate font-serif">
      ...
      <Button
       asChild
       size="default"
       className="bg-secondary dark:bg-[#7a6466] hover:bg-[#997b7e] dark:hover:bg-[#8f7577] text-white shadow-lg group w-full md:w-auto"
      >
       <Link href="/">
     ...
     <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
      <StatsCard
       ...
       value={pendingCount}
       color="bg-card text-foreground dark:text-card-foreground"
      />
      <StatsCard
       icon={MessageSquare}
      ...
      <StatsCard
       ...
       color="bg-primary text-white dark:bg-muted"
      />
      <StatsCard
       icon={CheckCircle}
       label={t('exchanges.stats.success_rate')}
       value={`${successRate}%`}
       color="bg-secondary text-white dark:bg-[#7a6466]"
      />
     </div>
    </div>
    ...
    <div className="max-w-4xl mx-auto">
         ...
         {pendingExchanges
          ...
          .map((exchange) => (
           <div key={exchange.id} className="relative">
            <div className="absolute -left-2 top-0 bottom-0 w-1 bg-card rounded-full shadow-lg"></div>
            <ExchangeItem
             exchange={exchange}
             onClick={() => handleExchangeClick(exchange)}
            />
     ...
     {/* Tips Section */}
     {exchanges.length > 0 && (
      <Card className="mt-6 md:mt-8 p-4 md:p-6 bg-card border-card shadow-lg">
       ...
       <ul className="space-y-2 text-xs md:text-sm text-muted-foreground font-serif">
